 ...
 *
 * Features:
 * - Intent classification (greeting, question, emotion_share, etc.)
 * - Emotion detection (happy, sad, angry, etc.)
 * - Personality engine with Korean speech patterns
 * - Conversation memory with user profiling
 * - Integration with SimGame for strategy selection
 *
 * @example
...
(function(global, factory) {
    ...
    class Retriever {
        ...
        /**
         * Load response data from URL or array
         * @param {string|Array|Object} data - URL, array, or object with responses
            ...
            if (typeof data === 'string') {
                try {
                    ...
                    const json = await retry(async () => {
                        ...
                        if (!response.ok) {
                            throw new ChatError(`Failed to fetch: ${response.status}`, 'FETCH_ERROR');
                        }
                        return response.json();
                    }, 3, 500);
                    this.responses = json.responses || json;
                } catch (e) {
                    console.warn('Failed to load responses after retries:', e);
                    this._loadBuiltIn();
                }
        ...
        // Retrieve with strategy metadata for game engine
        retrieveWithStrategy(input, topK = 10) {
        ...
        _inferStrategyFromEntry(entry) {
            // Infer strategy from entry properties
    ...
    class Engine {
        constructor(options = {}) {
            ...
            // Game-theoretic AI engine integration
        ...
        async initialize(responsesUrl) {
            ...
            // Initialize game engine if SimGame is available
            if (this.useGameTheory && typeof SimGame !== 'undefined') {
                this.gameEngine = SimGame.create({
                    storageKey: 'simchat_game',
                    autoSaveInterval: 30000
                });
                console.log('SimChat: Game-theoretic AI engine initialized');
            }
        ...
        async respond(userMessage) {
            ...
            // Process feedback from previous interaction
            if (this.gameEngine && this.lastStrategy && this.lastUserMessage) {
                this.gameEngine.processFeedback(this.lastStrategy, userMessage);
            }

            // Update game state
            if (this.gameEngine) {
                const emotionMap = { happy: 'positive', sad: 'negative', angry: 'negative',
                                     loving: 'positive', excited: 'positive' };
                this.gameEngine.updateState(userMessage, emotionMap[emotion.emotion] || 'neutral');
            }

            this.memory.extractUserName(userMessage);
            this.memory.addTurn('user', userMessage, { intent: intent.intent, emotion: emotion.emotion });

            // Generate response with game-theoretic selection
            ...
            // Store for next feedback cycle
            this.lastStrategy = strategy;
            this.lastUserMessage = userMessage;

            this.memory.addTurn('assistant', response, { emotion: responseEmotion, strategy });
            this.memory.save();
        ...
        async _generateWithStrategy(message, intent, emotion) {
            // Get candidates from retriever
            const candidates = this.retriever.retrieveWithStrategy(message, 10);
            let response, strategy = 'casual';

            // Use game engine to select best response if available
            if (this.gameEngine && candidates.length > 0) {
                const selected = this.gameEngine.selectResponse(candidates);
                if (selected) {
        ...
        _inferStrategy(intent, emotion) {
            // Infer strategy from intent and emotion
        ...
        applyFeedback(reward) {
            if (this.gameEngine && this.lastStrategy) {
                this.gameEngine.applyReward(this.lastStrategy, reward);
            }
        }
    }
