 ...
 * - Dense, Embedding, GRU, LayerNorm layers
 * - Multiple activation functions (ReLU, GELU, Softmax, etc.)
...
(function(global, factory) {
     ...
     * @private
     * @param {number[]} shape - Shape to validate
    ...
    class Tensor {
        /**
         * Create a new Tensor
         * @param {Float32Array|number[]|number} data - Data array, nested array, or fill value
         * @param {number[]} shape - Shape of the tensor [dim1, dim2, ...]
         * @throws {Error} If data type is invalid or shape doesn't match data size
         * @example
         * // From Float32Array
         * const t1 = new Tensor(new Float32Array([1, 2, 3, 4]), [2, 2]);
         *
         * // From nested array
         * const t2 = new Tensor([[1, 2], [3, 4]], [2, 2]);
         *
         * // Fill with value
         * const t3 = new Tensor(0, [3, 3]); // 3x3 zeros
         */
        constructor(data, shape) {
            ...
            if (data instanceof Float32Array) {
                ...
                throw new Error(`Invalid data type for Tensor: expected Float32Array, Array, or number, got ${typeof data}`);
            }
            ...
            /** @type {number[]} Shape of the tensor */
            this.shape = shape;
            /** @type {number[]} Strides for indexing */
            this.strides = this._computeStrides(shape);
            /** @type {number} Total number of elements */
         ...
         * @param {...number} indices - Indices for each dimension
         * @returns {Tensor} This tensor for chaining
         */
        set(value, ...indices) {
            let offset = 0;
            for (let i = 0; i < indices.length; i++) {
         ...
         * @param {number[]} newShape - New shape (must have same total size)
         * @returns {Tensor} New tensor with specified shape
         * @throws {Error} If new shape has different total size
         */
        reshape(newShape) {
         ...
         * @returns {Tensor} New transposed tensor
         * @throws {Error} If tensor is not 2D
         */
        transpose() {
         ...
         * @param {Tensor|number} other - Tensor or scalar to add
         * @returns {Tensor} New tensor with sum
         */
        add(other) {
         ...
         * @param {Tensor|number} other - Tensor or scalar to subtract
         * @returns {Tensor} New tensor with difference
         */
        sub(other) {
            if (other instanceof Tensor) {
                const result = new Float32Array(this.size);
         ...
         * @param {Tensor|number} other - Tensor or scalar to multiply
         * @returns {Tensor} New tensor with product
         */
        mul(other) {
            if (other instanceof Tensor) {
                if (this.size !== other.size) {
         ...
         * @param {Tensor} other - Right-hand tensor [K, N]
         * @returns {Tensor} Result tensor [M, N]
         * @throws {Error} If tensors are not 2D or dimensions don't match
         */
        matmul(other) {
    ...
    class Dense extends Layer {
         ...
         * @param {string} [options.activation='none'] - Activation function name
         * @param {boolean} [options.bias=true] - Whether to use bias
         * @throws {Error} If dimensions are not positive integers
         */
        constructor(inputDim, outputDim, options = {}) {
            super();
            validatePositiveInt(inputDim, 'inputDim');
            validatePositiveInt(outputDim, 'outputDim');
    ...
    class Embedding extends Layer {
         ...
         * @param {number} numEmbeddings - Vocabulary size
         * @param {number} embeddingDim - Embedding dimension
         * @param {Object} [options] - Layer options
         * @param {number} [options.paddingIdx] - Index for padding token (zeros)
         * @throws {Error} If dimensions are not positive integers
         */
        constructor(numEmbeddings, embeddingDim, options = {}) {
            super();
            validatePositiveInt(numEmbeddings, 'numEmbeddings');
            validatePositiveInt(embeddingDim, 'embeddingDim');
    ...
    class GRU extends Layer {
         ...
         * Create a GRU layer
         * @param {number} inputDim - Input dimension
         * @param {number} hiddenDim - Hidden state dimension
         * @param {Object} [options] - Layer options
         * @param {number} [options.numLayers=1] - Number of stacked layers
         * @param {boolean} [options.bidirectional=false] - Bidirectional mode
         * @throws {Error} If dimensions are not positive integers
         */
        constructor(inputDim, hiddenDim, options = {}) {
            super();
            validatePositiveInt(inputDim, 'inputDim');
            validatePositiveInt(hiddenDim, 'hiddenDim');
    ...
    return {
        ...
        // Layer factory functions (convenience)
