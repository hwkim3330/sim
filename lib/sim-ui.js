 ...
 * with proper resource cleanup and accessibility support.
 *
 * Features:
 * - Animated character with emotions and particles
 * - Chat interface with auto-scroll
 * - Voice synthesis with selectable voices
 * - Speech recognition support
 * - Proper dispose/cleanup patterns
 *
 * @example
 * const character = new SimUI.Character('#character');
...
(function(global, factory) {
    ...
    /**
     * Validate DOM element or selector
     * @private
     */
    function validateElement(value, name) {
        if (typeof value === 'string') {
            const el = document.querySelector(value);
            if (!el) {
                throw new Error(`${name}: element "${value}" not found`);
            }
            return el;
        }
        if (!(value instanceof Element)) {
            throw new TypeError(`${name} must be a DOM element or selector string`);
        }
        return value;
    }
    ...
    return {
        ...
        // Validation helper (exposed for external use)
        validateElement
    };
}));
