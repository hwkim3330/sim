...
(function(global) {
  ...
  class Resonator {
    ...
    setParams(freq, bandwidth) {
      ...
      const r = Math.exp(-Math.PI * T * bandwidth);
  ...
  class KlattSynthesizer {
    ...
    normalize(samples) {
      ...
      if (peak > 0) {
        return samples.map(s => s / peak * 0.8);
      }
      return samples;
    }
  ...
  class SimiTTS {
    ...
    async sing(melody, onProgress) {
      ...
      // Normalize
      const peak = Math.max(...allSamples.map(Math.abs));
      const normalized = peak > 0 ? allSamples.map(s => s / peak * 0.8) : allSamples;
